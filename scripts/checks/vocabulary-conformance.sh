#!/bin/sh
# Target-vocabulary conformance check — the control-repo copy of the
# dev-environment-facade skill's reference checker. The spec is that skill's
# "Enforcement" section; the canonical name list is vocabulary.txt. The
...
#
# Two copies exist ON PURPOSE, serving different consumers: the methodology
# repo's copy is read by an agent alongside the skill prose, this one is
# executed by gates. A repo adopts one tier or the other, never both. The
# ONLY difference is manifest resolution (below): the methodology copy
# defaults to a repo-relative vendored path, because a repo that vendors the
# skill already has the manifest; this copy defaults to the manifest shipped
# beside it, because a remote pre-commit consumer has no vendored copy to
# point at — that default is exactly what lets this be consumed at a pinned
# rev without vendoring anything.
#
# The vocabulary is a SHARED cross-repo standard, so the manifest travels
# with the checker rather than being copied into each consumer. Override
# with VOCABULARY_MANIFEST (or argv[1]) only for a repo that needs a
# deliberate variant.
#
# Checks:
...
set -u
# Resolve this script's own directory BEFORE cd-ing into the consuming repo --
# afterwards a relative $0 no longer resolves, and pre-commit runs the hook from
# its own clone, which is never inside the repo being checked.
script_dir=$(CDPATH='' cd -- "$(dirname -- "$0")" && pwd) || exit 2
cd "$(git rev-parse --show-toplevel)" || exit 2
LC_ALL=C; export LC_ALL
manifest="${1:-${VOCABULARY_MANIFEST:-$script_dir/vocabulary.txt}}"
